...
const isDraft = (s: string) => s === 'draft' || s === 'generated'
...
function computeStats(
  ...
  const pending    = working.filter((i) => !isPaid(i.status)).reduce((s, i) => s + Number(i.grand_total), 0)
...
export default function DashboardPage() {
  ...
  const companyStats = useMemo(() => {
    ...
    for (const inv of data.invoices) {
      ...
      s.invoiceCount++
      if (!isPaid(inv.status)) s.pending += Number(inv.grand_total)
